...
class DatabaseManager:
    ...
    def get_experiment_by_id(self, experiment_id):
        try:
            ...
            return None
    def get_experiment_type_id_by_experiment_id(self, experiment_id):
        try:
            self.cursor.execute(
            "SELECT experiment_type_id FROM experiments WHERE experiment_id = ?",
            (experiment_id,),
            )
            row = self.cursor.fetchone()
            return row[0] if row else None
        except sqlite3.Error as e:
            print(f"Error fetching experiment_type_id by experiment_id: {e}")
            return None
    def get_experiment_type_name_by_id(self, experiment_type_id):
        return self.get_experiment_type_by_id(experiment_type_id)
